...
impGDPUpdated <- impGDP %>% 
  ...
  ungroup() %>% 
  left_join(isoContinent[, c("country_1", "cont", "sub_region")], by = "country_1") %>% 
  mutate(sub_region = factor(sub_region, levels = c("Eastern Asia", "Latin America and the Caribbean", "Northern Africa", "South-eastern Asia", "Sub-Saharan Africa", "Western Asia")), order = if_else(sub_region == "Latin America and the Caribbean", "A", if_else(sub_region == "South-eastern Asia", "B", if_else(sub_region == "Sub-Saharan Africa", "C", "o")))) 


ggplot(impGDPUpdated, aes(x = fct_reorder(country_1, order), y = scenario, fill = gdpPercImp)) +
    geom_tile() +
    theme_classic() +
