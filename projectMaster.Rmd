...
ggplot(data = p2 %>% 
  ...
  scale_color_manual(values = c("#c1cef7", "#a1b0e3" ,"#607ce0", "#2246c7",  "#061c2b")) + 
    # scale_fill_manual(values = c("#c1cef7", "#a1b0e3" ,"#607ce0", "#2246c7",  "#061c2b")) + 
...
"El Salvador"      ,     "French Guiana"    ,     "Guatemala"      ,       "Guyana", "Haiti", 
 "Honduras"      ,        "Jamaica"           ,    "Mexico", "Suriname"  ,           "Nicaragua"            ,"Peru",   "Panama", "Puerto Rico", "Uruguay"  ,             "Venezuela")
# Retrievethe map data
vietnam <- map_data("world", region  = "Vietnam")

library(ggrepel)

# Compute the centroid as the mean longitude and lattitude
# Used as label coordinate for country's names
region.lab.data <- vietnam %>%
  group_by(region) %>%
  summarise(long = mean(long), lat = mean(lat))

ggplot(vietnam, aes(x = long, y = lat)) +
  geom_polygon(aes(group = group), fill = "#2246c7", color = "#2246c7", size = 0.2) +
  coord_sf(crs = "+proj=laea +lat_0=52 +lon_0=10 +x_0=4321000 +y_0=3210000 +ellps=GRS80 +units=m +no_defs ") +
  theme(panel.background = element_rect(fill = "#061c2b"))

bahamas <- map_data("world", region  = "Bahamas")

region.lab.data <- bahamas %>%
  group_by(region) %>%
  summarise(long = mean(long), lat = mean(lat))

ggplot(bahamas, aes(x = long, y = lat)) +
  geom_polygon(aes(group = group), fill = "#2246c7", color = "#2246c7", size = 0.2) +
  coord_sf(crs = "+proj=laea +lat_0=52 +lon_0=10 +x_0=4321000 +y_0=3210000 +ellps=GRS80 +units=m +no_defs ") +
  theme(panel.background = element_rect(fill = "#061c2b"))
```
...
impGDPUpdated <- impGDP %>% 
  ...
  mutate(scenario = factor(scenario, levels = c("5", "4", "3", "2", "1"))) %>% 
  group_by(country_1) %>% 
  mutate(meanImp = mean(gdpPercImp), takeYN = if_else(meanImp >= 0.05, "Y", "N")) %>% 
  filter(takeYN == "Y") %>% 
  ungroup() %>% 
  left_join(isoContinent[, c("country_1", "cont", "sub_region")], by = "country_1") %>% 
  mutate(sub_region = factor(sub_region, levels = c("Eastern Asia", "Latin America and the Caribbean", "Northern Africa", "South-eastern Asia", "Sub-Saharan Africa", "Western Asia")), order = if_else(sub_region == "Latin America and the Caribbean", "A", if_else(sub_region == "South-eastern Asia", "B", if_else(sub_region == "Sub-Saharan Africa", "C", "o"))), country_2 = if_else(country_1 == "MMR", "Myanmar", country_2)) 


ggplot(impGDPUpdated, aes(x = fct_reorder(country_2, order), y = scenario, fill = gdpPercImp)) +
    geom_tile() +
    theme_classic() +
    # geom_text(aes(label = scales::number(gdpPercImp*100, accuracy = 1), color = gdpPercImp)) +
    theme(axis.text.x = element_text()) +
    scale_fill_gradient(low = "white" ,high = "#061c2b", labels = scales::percent) +
    # theme(axis.text.x = element_text(angle = 30, vjust = 0.6)) +
    scale_color_gradient(high = "white" ,low = "#061c2b") +
    labs(fill = "Impacted GDP")
